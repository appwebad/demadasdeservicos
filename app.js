const GOOGLE_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbxtxIMTbJcfSda-7P51EDv74XJ3Y8wQ52pZLmal9MCYcNPWr2wrgcO2RdKwu20YnL9L/exec";
...
function getCurrentUser() {
  ...
  const collection =
    state.session.type === "candidate" ? state.candidates : state.companies;
...
function renderCandidateDashboard() {
  ...
  document.getElementById("candidateVacanciesCount").textContent = String(
    state.jobs.length
  );

  document.getElementById("candidateApplicationsCount").textContent = String(
    applications.length
  );
  ...
  state.jobs
    ...
    .forEach((job) => {
      ...
      const applicantsCount = state.applications.filter(
...
function renderCompanyDashboard() {
  ...
  document.getElementById("companyApplicantsCount").textContent = String(
...
function buildMetaGrid(job) {
  ...
  const fields = [
    {
      label: "Valor diária",
      value: formatMoney(job.dailyRate)
    },
    {
      label: "Pagamento",
      value: formatDate(job.paymentDate)
    },
    {
      label: "Local",
      value: job.location
